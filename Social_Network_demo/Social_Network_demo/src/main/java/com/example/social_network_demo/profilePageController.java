...
public class profilePageController implements ProfilBilgiler{
    @FXML
    ...
    public void GirisController(String username,String password) throws FileNotFoundException, UserAbsent, wrongPassword, FileError {
        loginİslem(username,password);
    }

    public void loginİslem(String username,String pass) throws FileNotFoundException, UserAbsent, wrongPassword, FileError {
        ...
        if (file.exists()) {
                ...
                while(scanner.hasNext()){
                    bilgi.add(scanner.nextLine());
                }
                scanner.close();
                if(bilgi.get(4).equals(pass)){
                bilgiler();

                }else{
                throw new wrongPassword(pass);
                }

        }else {
            throw new UserAbsent(username);
        }
    }
    ...
    public void AddFriendButtonClicked(ActionEvent event) throws IOException {
        ...
        try {
            friendController.Bilgiler(bilgi.get(3),bilgi.get(4));
            friendController.KullaniciAl();
            stage = (Stage) ((Node)event.getSource()).getScene().getWindow();
            scene = new Scene(root);
            stage.setScene(scene);
            stage.show();
    ...
    public void editMyProfileButtonClicked(ActionEvent event) throws IOException {
        ...
        try {
            editController.Bilgiler(bilgi);
            editController.UserWrite();
            stage = (Stage) ((Node)event.getSource()).getScene().getWindow();
            scene = new Scene(root);
            stage.setScene(scene);
            stage.show();
